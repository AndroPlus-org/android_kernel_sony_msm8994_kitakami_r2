...
#define bcl_prev_num_online_cpus()	cpumask_weight(bcl_cpu_online_mask)

static int bcl_cpumask_outdated(void)
{
	return bcl_prev_num_online_cpus() != num_online_cpus();
}
...
static void __ref bcl_handle_hotplug(struct work_struct *work)
{
	...
	if (bcl_cpumask_outdated() || cpumask_empty(bcl_cpu_online_mask))
