...
static int a53pvs_get(void *data, u64 *val)
{
	*val = 0;
...
static int a57pvs_get(void *data, u64 *val)
{
	*val = 0;
...
DEFINE_SIMPLE_ATTRIBUTE(a53pvs_ops, a53pvs_get, NULL, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(a57pvs_ops, a57pvs_get, NULL, "%llu\n");
...
static int cpuinfo_debugfs_init(void)
{
	...
	if (debugfs_create_file("A57-Speedbin", S_IRUGO,
		...
		goto init_failed;
	if (debugfs_create_file("A53-PVS", S_IRUGO,
			cpuinfo_rootdir, NULL, &a53pvs_ops) == NULL)
		goto init_failed;
	if (debugfs_create_file("A57-PVS", S_IRUGO,
			cpuinfo_rootdir, NULL, &a57pvs_ops) == NULL)
		goto init_failed;
